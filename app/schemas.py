...
class ReviewCreate(BaseModel):
    ...
    comment: Optional[str] = Field(None, max_length=500, description="Текст отзыва (необязательно)")
...
class Cart(BaseModel):
    ...
    total_price: int = Field(..., ge=0, description="Общая стоимость товаров")
...
class Order(BaseModel): 
    ...
    updated_at: datetime = Field(..., description="Когда заказ последний раз обновлялся")
